json.array!(@stores) do |store|
  ...
  unless store.image.url.nil?
	  json.thumb self.request.domain+store.image.square.url
	  json.image self.request.domain+store.image.url
	end
  json.cuisines do 
  	json.array!(store.cuisines) do |cuisine|
  		json.cuisine_id cuisine.id
  		unless cuisine.image.url.nil?
  			json.thumb self.request.domain+cuisine.image.square.url
  			json.image self.request.domain+cuisine.image.url
  		end
